...
def worker_init_0(l):
    return [np.random.seed(l), random.seed(l), torch.manual_seed(l)]


def worker_init_2(l):
    return [np.random.seed((2 + l)), random.seed(2 + l), torch.manual_seed(2 + l)]


def worker_init_4(l):
    return [np.random.seed((4 + l)), random.seed(4 + l), torch.manual_seed(4 + l)]


def worker_init_6(l):
    return [np.random.seed((6 + l)), random.seed(6 + l), torch.manual_seed(6 + l)]


def worker_init_8(l):
    return [np.random.seed((8 + l)), random.seed(8 + l), torch.manual_seed(8 + l)]
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for sample in dataloaders[phase]:
                ...
                sampleweights = torch.from_numpy(
                    classweights1[np.searchsorted(classes1, labels.data.numpy()[:, 0:1])]).float().to(device).view(-1,
                                                                                                                   1)
                ...
                running_pred_label = np.concatenate((running_pred_label, np.concatenate(
                    [outputs.data.cpu().numpy(), labels.data.cpu().numpy()], axis=1)))

            pred_test = running_pred_label[:, 0:10]
            label_test = running_pred_label[:, 10:]

            sampleweights = weights[phase]['classweights'][
                np.searchsorted(weights[phase]['classes'], label_test[:, 0:1])]
            ...
            print('{} loss: {:.4f} weighted MSE: {:.4f} weighted MAE: {:.4f}'.format(phase, epoch_loss,
                                                                                     epoch_weighted_mse,
                                                                                     epoch_weighted_mae))
...
def test_model(model, dataloader, weights, result_dir):
    ...
    for sample in dataloader:
        ...
        for curr_id in range(len(imageids)):
            ...
            f = open(os.path.join(result_dir, subjids[curr_id], videoids[curr_id], imageids[curr_id][:-4] + '.txt'),
                     "w")
            f.write(str(outputs.data.cpu().numpy()[curr_id][0]))
            f.close()
            np.savez(os.path.join(result_dir, subjids[curr_id], videoids[curr_id], imageids[curr_id][:-4] + '.npz'),
                     output=outputs.data.cpu().numpy()[curr_id], feat_map=my_embedding.data.cpu().numpy()[curr_id])

        # statistics
        running_pred_label = np.concatenate(
            (running_pred_label, np.concatenate([outputs.data.cpu().numpy(), labels.data.cpu().numpy()], axis=1)))
    ...
    print(
        '{} Acc: {:.4f} Weighted Acc: {:.4f} MSE: {:.4f} Weighted MSE: {:.4f} MAE: {:.4f} Weighted MAE: {:.4f}'.format(
            'test',
...
if __name__ == '__main__':
    ...
    transformations0 = transforms.Compose([transforms.Lambda(worker_init_0)])
    transformations2 = transforms.Compose([transforms.Lambda(worker_init_2)])
    transformations4 = transforms.Compose([transforms.Lambda(worker_init_4)])
    transformations6 = transforms.Compose([transforms.Lambda(worker_init_6)])
    transformations8 = transforms.Compose([transforms.Lambda(worker_init_8)])

    transformations = {0: transformations0, 2: transformations2, 4: transformations4, 6: transformations6,
                       8: transformations8}
    ...
    for rseed in [0, 2, 4, 6, 8]:
        ...
        # TODO: replace image_dir and label_dir directories if needed, to point to the dataset
        image_dir = "./croptargetSample/Images"
        label_dir = "./croptargetSample/Labels"
        ...
        fold_size = 1  # Initially 5
        folds = []
        for i in range(3):  # Initially 5
        ...
        for subj_left_id, subj_left_out in enumerate(folds):
            ...
            datasets = {x: McMasterDataset(image_dir, label_dir, val_subj, test_subj, x, data_transforms[x]) for x in
                        ['train', 'val', 'test']}
            ...
            dataloaders_dict = {
                x: torch.utils.data.DataLoader(datasets[x], batch_size=batch_size, shuffle=shuffle[x], num_workers=4,
                                               worker_init_fn=transformations[rseed]) for x in
                ['train', 'val', 'test']}
            ...
            if os.path.isfile('./models_sf' + str(rseed) + '/' + str(subj_left_id) + '.pth'):
                model_ft.load_state_dict(
                    torch.load('./models_sf' + str(rseed) + '/' + str(subj_left_id) + '.pth', map_location=device))
            else:
                model_ft, train_hist, val_hist = train_model(model_ft, dataloaders_dict, criterion, optimizer_ft,
                                                             num_epochs=num_epochs, weights=weights,
            ...
            pred_train, label_train = test_model(model_ft, dataloaders_dict['train'], weights['train'],
                                                 './results_sf' + str(rseed) + '/' + str(subj_left_id))
            ...
            pred_test, label_test = test_model(model_ft, dataloaders_dict['test'], weights['test'],
                                               './results_sf' + str(rseed) + '/' + str(subj_left_id))
            np.savez('./results_sf' + str(rseed) + '/' + str(subj_left_id) + '.npz', pred=pred_test, label=label_test,
                     pred_val=pred_val, label_val=label_val, pred_train=pred_train, label_train=label_train)
